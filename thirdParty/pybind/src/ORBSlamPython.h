...
public:
    ...
    bool initialize(bool icpMix = false);
    bool isRunning();
    ...
    bool trackWithIcpPose(cv::Mat image, cv::Mat depthImage, cv::Mat IcpPose_t1_t0, double timestamp);
    bool trackWithOrbFeature(cv::Mat image, cv::Mat depthImage, double timestamp);
