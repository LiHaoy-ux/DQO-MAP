...
def read_pose_t0(args):
    ...
    if data_type == "Replica":
        ...
        pose_t0_c2w = quaternion_to_rotation_matrix(pose_t0_c2w[1:])
    else:
        pose_t0_c2w = np.eye(4)
    return pose_t0_c2w
...
def main():
    ...
    obj_models = [model for model in select_models if "obj" in model]
    ...
    for obj_model in obj_models:
        ...
        #     pcd_path = select_ply
        if "41" in obj_model:
            print("select model is cup")
            mesh_path="/home/lihy/3DGS/RTG-SLAM/data/RO-MAP_data/room/gt_mesh/cup.ply"
        elif "58" in obj_model:
            print("select model is bluebell")
            mesh_path = "/home/lihy/3DGS/RTG-SLAM/data/RO-MAP_data/room/gt_mesh/bluebell.ply"
        elif "63" in obj_model:
            print("select model is laptop")
            mesh_path = "/home/lihy/3DGS/RTG-SLAM/data/RO-MAP_data/room/gt_mesh/laptop.ply"
        elif "73" in obj_model:
            print("select model is book")
            mesh_path = "/home/lihy/3DGS/RTG-SLAM/data/RO-MAP_data/room/gt_mesh/book.ply"
        else: print("select model is unknown")
        ...
        df = pd.DataFrame(total_loss)  # ?计算得到结果
        print(df.mean())
        total_loss.clear()#?清空total_loss
