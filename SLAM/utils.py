...
def rot_compare(prev_rot: np.array, curr_rot: np.array):
    ...
    cos_theta = (np.trace(rot_diff) - 1) / 2 #使用trace计算旋转角度
    rad_diff = np.arccos(cos_theta)#转为角度
    theta_diff = np.rad2deg(rad_diff)#转为弧度
    return rad_diff, theta_diff


def trans_compare(prev_trans: np.array, curr_trans: np.array):
    trans_diff = prev_trans - curr_trans
    l1_diff = np.linalg.norm(trans_diff, ord=1)#计算L1范数
    l2_diff = np.linalg.norm(trans_diff, ord=2)#计算L2范数
...
def feature_gradient(img, normalize_gradient=True):
    ...
    #定义sobel卷积核，.view(1, 1, 3, 3)重塑成[1,1,3,3]为了适应卷积操作
    ...
    #F.pad是填充操作，（1，1，1，1）在四周填充一个像素，replicate采用复制的方式
    img_pad = F.pad(img_permuted, (1, 1, 1, 1), mode='replicate')
    img_dx = F.conv2d(img_pad, wx, stride=1, padding=0).squeeze().permute(1, 2, 0)  # [h, w, c] x方向梯度
    img_dy = F.conv2d(img_pad, wy, stride=1, padding=0).squeeze().permute(1, 2, 0)  # [h, w, c] y方向梯度
...
def compute_normal_map(vertex_map):
    ...
    mag = torch.norm(normal, p=2, dim=-1, keepdim=True)#计算L2范数 [h,w,1]
    ...
    normal = torch.where(invalid_mask[..., None], zero_normal, normal) #根据invalid_mask的值选择zero_normal 和normal的值
...
def compute_confidence_map(normal_map, intrinsic):
    ...
    view_normal_dist = torch.abs(F.cosine_similarity(normal_map, proj_map, dim=-1))#得到像素法向量和相机投影向量之间的相似度
    return view_normal_dist[..., None]

#!增加对语义的支持
def sample_pixels(
    vertex_map,#世界坐标系下的点
    normal_map,#世界坐标系下的法向量
    color_map,
    uniform_sample_num,#采样像素为50000
    select_mask=None, #传入的是深度图中值大于0的地方
    semantic_color=None,
    instace_img = None
):
    assert uniform_sample_num >= 0
    if uniform_sample_num == 0:
        return devF(torch.empty(0)), devF(torch.empty(0)), devF(torch.empty(0)),devF(torch.empty(0)),devF(torch.empty(0))

    # 1. compute local data（确定采样点的位置）
    ...
    # ### 2024-12-11 根据法向量进行筛选采样点
    invalid_normal_mask = torch.where(normal_map.sum(dim=-1) == 0)
    select_mask[invalid_normal_mask] = False
    ###2024-12-11 使用实例分割结果构建新的loss 创建物体的mask
    if instace_img is not None:
        instance_mask = torch.where(instace_img.sum(dim=-1) == 0)
        select_mask[instance_mask] = False

    if uniform_sample_num > select_mask.sum():
        uniform_sample_num = select_mask.sum()

    select_mask = select_mask.flatten()
    vertexs = vertex_map.view(-1, 3)[select_mask]#筛选
    colors = color_map.view(-1, 3)[select_mask]
    normals = normal_map.view(-1, 3)[select_mask]

    #torch.randperm(vertexs.shape[0])：随机生成0-》n-1，从中选择前uniform_sample_num个
    ...
    if instace_img is not None:
        instance_img = instace_img.view(-1, 3)[select_mask]
        instance_color_uniform = instance_img[samples]
    else:
        instance_color_uniform = None

    #?语义信息
    if semantic_color is not None:
        semantic_color = semantic_color.view(-1, 3)[select_mask]
        semantic_color_uniform = semantic_color[samples]
    else:
        semantic_color_uniform = None
    return(
        ...
        ##2025-01-08注释，看看是否能解决 not enough values to unpack (expected 5, got 4)问题
        #semantic_color_uniform.view(uniform_sample_num, 3) if semantic_color is not None else None,
        None,
        instance_color_uniform.view(uniform_sample_num, 3) if instace_img is not None else torch.empty(0, 3),
        )
...
def prepare_cfg(
    ...
    data_path = args.source_path#数据所在位置
    save_path = args.save_path#数据保存位置
    os.makedirs(save_path, exist_ok=True)
    #把本次运行使用的参数保存到confg.yaml文件中
...
def build_vertex_pyramid(depth, pyramid_builder, K):
    H, W = depth.shape[:2]
    depth_pyramid = pyramid_builder(depth.view(1,1,H,W))#获得各层金字塔里的图片(3层)
    vertex_pyramid = []
    for i in range(len(depth_pyramid)):
        H_sub, W_sub = depth_pyramid[i].shape[2:4]#获得每层的长和宽
        downscale = 1 / 2**(len(depth_pyramid)-i-1)
        K_downscale = K * downscale#对内参矩阵也进行缩放
        K_downscale[2,2] = 1.0
        #分别得到相机坐标系下坐标
...
def move_to_gpu(frame):
    ...
    if frame.semantics is not None:
        frame.semantics = devF(frame.semantics)
    #2024-11-20 加入object_img
    if frame.object_img is not None:
        frame.object_img = devF(frame.object_img)
    ###2024-12-11 使用实例分割结果构建新的loss
    if frame.instance_img is not None:
        frame.instance_img = devF(frame.instance_img)
...
def move_to_cpu(frame):
    ...
    if frame.semantics is not None:
        frame.semantics = frame.semantics.to("cpu")
    if frame.object_img is not None:
        frame.object_img = frame.object_img.to("cpu")
    if frame.instance_img is not None:
        frame.instance_img = frame.instance_img.to("cpu")
...
def move_to_gpu_map(keymap):
    ...
    if keymap["semantics_color"] is not None:
        keymap["semantics_color"] = devF(keymap["semantics_color"])
    if keymap["instance_img"] is not None:
        keymap["instance_img"] = devF(keymap["instance_img"])
...
def move_to_cpu_map(keymap):
    ...
    if keymap["semantics_color"] is not None:
        keymap["semantics_color"] = keymap["semantics_color"].detach().cpu()
    if keymap["instance_img"] is not None:
        keymap["instance_img"] = keymap["instance_img"].detach().cpu()
...
#返回total_xyz在local_xyz组成的包围盒的点
def bbox_filter(local_xyz, total_xyz, padding=0.05):
    local_min = local_xyz.min(dim=0)[0] - padding #local_xyz.min(dim=0),返回最小值和最小值的索引，[0]取出最小值
