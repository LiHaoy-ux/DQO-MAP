...
class ICP(nn.Module):
    def __init__(
        ...
        self.normal_threshold = np.cos(np.deg2rad(normal_threshold))#20
...
def invH(H):
    ...
    if H.is_cuda:
        if torch.det(H) == 0:
            invH = torch.pinverse(H.cpu()).cuda()
        else:
...
class ImagePyramids(nn.Module):
    ...
    def __init__(self, scales, pool='avg'):
        ...
        if pool == 'avg':
            ...
            self.multiscales = [nn.MaxPool2d(1<<i, 1<<i) for i in scales]#池化层大小和步长都一样
        else:
...
class IcpTracker:
    def __init__(self, args):
        self.icp_trackers = []
        self.icp_downscales = args.icp_downscales#0.25 0.5 1.0
        self.icp_warmup_frames = args.icp_warmup_frames#0
        self.icp_use_model_depth = args.icp_use_model_depth#true
        for iters in args.icp_downscale_iters:#args.icp_downscale_iter 【5 5 5】
            self.icp_trackers.append(
                ICP(
                    iters,#5
                    distance_threshold=args.icp_distance_threshold,#0.1
                    normal_threshold=args.icp_normal_threshold,#20
                    damping=args.icp_damping,#0.0001
                    verbose=args.verbose,#false
                )
            )
        #range(a,-1,-1)第一个-1代表结束值（可以忽视），第二个-1代表是递减
        #设置多层金字塔
        self.depth_pyramid_builder = ImagePyramids(list(range(len(self.icp_downscales)-1,-1, -1)), "max")
        self.icp_sample_distance_threshold = args.icp_sample_distance_threshold#0.01
        self.icp_sample_normal_threshold = args.icp_sample_normal_threshold#0.01
        self.icp_fail_threshold = args.icp_fail_threshold#0.02
    ...
    def update_curr_status(self, depth_t1, K):
        ...
        self.vertex_pyramid_t1 = build_vertex_pyramid(depth_t1, self.depth_pyramid_builder, self.K)#分别得到金字塔每一层图像
        self.normal_pyramid_t1 = build_normal_pyramid(self.vertex_pyramid_t1)#得到每一层法向量
